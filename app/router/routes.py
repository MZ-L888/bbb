    ...
    async def health_check(request: Request):
        ...
        try:
            ...
            try:
                key_manager = await get_key_manager_instance()
                keys_status = await key_manager.get_keys_by_status()
                valid_keys_count = len(keys_status["valid_keys"])
                total_keys_count = len(keys_status["valid_keys"]) + len(keys_status["invalid_keys"])
            except Exception as e:
                logger.warning(f"Key manager check failed: {str(e)}")
                valid_keys_count = 0
                total_keys_count = 0

            health_data = {
                "status": "healthy",
                "timestamp": request.headers.get("x-forwarded-for", "unknown"),
                "database": db_status,
                "keys": {
